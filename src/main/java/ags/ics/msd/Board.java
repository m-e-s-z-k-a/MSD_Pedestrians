...
public class Board extends JComponent implements MouseInputListener, ComponentListener {
    ...
    private void initialize(int length, int height) {
        ...
        for (int x = 1; x < points.length-1; ++x) {
            for (int y = 1; y < points[x].length-1; ++y)
            {
                points[x][y].addMooreNeighbor(points[x-1][y-1]);
                points[x][y].addMooreNeighbor(points[x-1][y]);
                points[x][y].addMooreNeighbor(points[x-1][y+1]);
                points[x][y].addMooreNeighbor(points[x][y-1]);
                points[x][y].addMooreNeighbor(points[x][y+1]);
                points[x][y].addMooreNeighbor(points[x+1][y-1]);
                points[x][y].addMooreNeighbor(points[x+1][y]);
                points[x][y].addMooreNeighbor(points[x+1][y+1]);

                points[x][y].addNeumannNeighbor(points[x-1][y]);
                points[x][y].addNeumannNeighbor(points[x+1][y]);
                points[x][y].addNeumannNeighbor(points[x][y-1]);
                points[x][y].addNeumannNeighbor(points[x][y+1]);
            }
        }
    }
